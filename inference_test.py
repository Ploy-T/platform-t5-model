...
if __name__ == '__main__':
    ...
    print("Loading model..")
    tokenizer = T5Tokenizer.from_pretrained("t5-base") if selection == 2 else AutoTokenizer.from_pretrained("results/bert")
    model = T5ForConditionalGeneration.from_pretrained("t5-base") if selection == 2 else BertForQuestionAnswering.from_pretrained("results/bert")
    print("Model loaded.")
    ...
    with torch.no_grad(): 
        if type == 2:
            #context: str = input()
            jsonData = json.load(open(os.path.join('data','All Star. HK. exp. 7.12.23-20980000.json')))
            context = jsonData['raw_text']
            print("context..")
